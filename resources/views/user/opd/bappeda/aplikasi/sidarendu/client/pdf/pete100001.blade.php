...
<h4 class="card-title">JUMLAH POPULASI TERNAK UNGGAS MENURUT KECAMATAN</h4>
    ...
    <tbody>
        ...
        @forelse ($pdfdata as $data)
            <tr>
                ...
                <td>{{ $data->itik }}</td>
                <td>{{ $data->manila }}</td>
                <td>{{ $data->ayam_buras + $data->ayam_ras_pedaging + $data->ayam_ras_petelur + $data->merpati + $data->ayam_burung_puyuh + $data->itik + $data->manila }}</td>
            </tr>
            <?php $no++; ?>
        @empty
            <tr>
